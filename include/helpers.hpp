...
static inline bool string_contains(const std::string& s1, const std::string& s2) {
  return (s1.find(s2) != std::string::npos);
}

static inline std::string num2s(unsigned num, bool is_hex = true) {
  char buf[16];
...
static inline std::string pixfmt_to_string(__u32 format) {
  switch (format) {
...
static inline std::vector<fs::path> get_video_device_files() {
